...
be one of doric, ef, or modus themes."
  (let ((theme (car custom-enabled-themes)))
    (cond ((or (and (boundp 'ef-themes-collection)
	            (member theme ef-themes-collection))
               (and (boundp 'modus-themes-collection)
	            (member theme modus-themes-collection)))
	     ...
	     (setq hl-todo-keyword-faces
		     ...
		     ("FIXME" . ,red)
		     ("OKAY" . ,cyan)))))
          ((and (boundp 'doric-themes-collection)
	        (member theme doric-themes-collection))
           (let* ((palette-var (intern (concat (symbol-name theme) "-palette")))
	          (palette (symbol-value palette-var))
	          (red (cadr (assoc 'fg-red palette)))
	          (green (cadr (assoc 'fg-green palette)))
	          (yellow (cadr (assoc 'fg-yellow palette)))
	          (cyan (cadr (assoc 'fg-cyan palette))))
	     (setq hl-todo-keyword-faces
	           `(("TODO" . ,red)
		     ("DONE" . ,green)
		     ("NOTE" . ,yellow)
    ...
    (condition-case nil
        (consult--read
         ...
         :lookup (lambda (selected &rest _)
                   ...
                   (or (and selected (car (memq selected (append ef-themes-collection doric-themes-collection modus-themes-collection))))
                              saved-theme))
...
(with-eval-after-load-all (
                           ;; doric-themes <-- see https://github.com/protesilaos/doric-themes/issues/14 about why this is commented out
                           ef-themes modus-themes consult calle24 hl-todo fontaine-org
                           )
  ;; (require 'doric-themes) <-- see https://github.com/protesilaos/doric-themes/issues/14 for why this is commented out
